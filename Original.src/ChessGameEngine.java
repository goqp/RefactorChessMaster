...
public class ChessGameEngine {
    ...
    public boolean playerHasLegalMoves(int playerNum) {
        ArrayList<ChessGamePiece> pieces = getPlayerPieces(playerNum);
    ...
    private ArrayList<ChessGamePiece> getPlayerPieces(int playerNum) {
        if (playerNum == 1) {
        ...
        return new ArrayList<>(); // Jugador inválido, se puede ajustar según tus necesidades.
    }
    ...
    private boolean selectedPieceIsValid() {
        if (currentPiece == null) {
            return false; // El usuario intentó seleccionar un cuadro vacío
        }
    
        int expectedColor = (currentPlayer == 2) ? ChessGamePiece.BLACK : ChessGamePiece.WHITE;
        return currentPiece.getColorOfPiece() == expectedColor;
    }
